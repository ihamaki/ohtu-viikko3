Feature: A new user account can be created if a proper unused username and password are given
  ...
  Scenario: creation is successful with valid username and password
    ...
    When username "ukkonooa" and password "salainen1" are entered
  ...
  Scenario: creation fails with valid username and too short password
    Given command new user is selected
    When username "ukko" and password "nooa" are entered
    Then system will respond with "new user not registered"

  Scenario: creation fails with valid username and password enough long but consisting of only letters
    Given command new user is selected
    When username "ukkonooa" and password "kunnonmies" are entered
    Then system will respond with "new user not registered"
